...
if __name__ == '__main__':
    ...
    for i in range(5):
        # a1) Move receiver forwards 1.5 meters while avoiding obstacles, then rotate 90 degrees
        encoder_vals1 = distspinner.move_robot(moving_x = False, distance = 1.1, diff = 0, ep_camera=ep_camera)  
        ep_chassis.drive_speed(x=0, y=0, z=sign*80, timeout=5)
        time.sleep(1)

        # a2) Move receiver horizontally to blue line
        encoder_vals2 = distspinner.move_robot(moving_x = True, distance = 0.8, diff = encoder_vals1[0], ep_camera=ep_camera)  

        # a3) Look for and move to lego 
        encoder_vals3 = distspinner.look_for_lego(ep_camera=ep_camera, is_Left=False)
        
        # a4) Grab lego
        distspinner.grab_lego(ep_gripper=ep_gripper, ep_arm=ep_arm)
        
        # a5) Rotate back towards starting point
        rotation = abs(int(abs(encoder_vals3[2]-encoder_vals2[2]))%360 - 180)  
        # if encoder_vals2[1] < 0:
        #     rotation = -rotation
        # print(rotation)
        ep_chassis.drive_speed(x=0, y=0, z=-sign*(rotation-10), timeout=5)
        time.sleep(1)

        # a6) Move receiver horizontally away from blue line, then rotate 90 degrees
        encoder_vals4 = distspinner.move_robot(moving_x = True, distance = 0.8, diff = encoder_vals3[0], ep_camera=ep_camera)  
        ep_chassis.drive_speed(x=0, y=0, z=-sign*80, timeout=5)
        time.sleep(1)

        # a7) Move receiver backwards 1.5 meters while avoiding obstacles
        encoder_vals5 = distspinner.move_robot(moving_x = False, distance = 1.1, diff = encoder_vals4[1], ep_camera=ep_camera)  
        ep_chassis.drive_speed(x=0, y=0, z=sign*80, timeout=5)
        time.sleep(1)

        # a8) Search for target
